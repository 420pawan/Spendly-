...
from database.queries import get_category_breakdown, get_recent_transactions, get_summary_stats, insert_expense
...
def add_expense():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render_template("add_expense.html", categories=EXPENSE_CATEGORIES, values=values, error="Amount must be a positive number.")
        if amount <= 0:
            return render_template("add_expense.html", categories=EXPENSE_CATEGORIES, values=values, error="Amount must be greater than zero.")
        if values["category"] not in EXPENSE_CATEGORIES:
            return render_template("add_expense.html", categories=EXPENSE_CATEGORIES, values=values, error="Select a valid category.")
        try:
            datetime.strptime(values["date"], "%Y-%m-%d")
        except ValueError:
            return render_template("add_expense.html", categories=EXPENSE_CATEGORIES, values=values, error="Enter a valid date.")
        if len(values["description"]) > 200:
            return render_template("add_expense.html", categories=EXPENSE_CATEGORIES, values=values, error="Description must be 200 characters or fewer.")

        insert_expense(session["user_id"], amount, values["category"], values["date"], values["description"] or None)
        return redirect(url_for("profile"))

    return render_template("add_expense.html", categories=EXPENSE_CATEGORIES, values=values, error=None)
