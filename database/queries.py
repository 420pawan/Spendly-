    ...
    return [
        {
            ...
            "percentage": (
                round((row["total"] / overall_total) * 100)
                if overall_total
                else 0
            ),
...
def get_expense_by_id(expense_id: int, user_id: int):
    """Return an expense only when it belongs to the supplied user."""
    connection = get_db()
    try:
...
def update_expense(
    ...
    """Update an expense only when it belongs to the supplied user."""
    connection = get_db()
    try:
        connection.execute(
            """
